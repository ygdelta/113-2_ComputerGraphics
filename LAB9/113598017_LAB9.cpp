...
void PrepareTriangleFilling(void);
...
void PrepareTriangleFilling(void) {
    if (endpointCount == 3) {
        ...
        trianglePoints = (Point*)malloc(trianglePointCount * sizeof(Point));
...
void AnimationTimer(int value) {
    if (animationActive && currentTrianglePoint < trianglePointCount) {
        Point p = trianglePoints[currentTrianglePoint];
        
        // Draw the current point
        currentTrianglePoint++;
...
void FillTriangle(void) {
    if (endpointCount == 3) {
        ...
        // Draw only the points processed so far in the animation
        for (int i = 0; i < currentTrianglePoint && i < trianglePointCount; i++) {
            Point p = trianglePoints[i];
            float x1 = p.x - size - 0.5f;
            float y1 = p.y - size - 0.5f;
            glBegin(GL_QUADS);
                glVertex2f(x1,     y1);
                glVertex2f(x1 + 1, y1);
                glVertex2f(x1 + 1, y1 + 1);
    ...
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        if (gx>=0 && gx<2*size+1 && gy>=0 && gy<2*size+1) {
            if (endpointCount < 3) {
                ...
                // When we have all 3 points, prepare the triangle filling
                if (endpointCount == 3) {
                    PrepareTriangleFilling();
                }
            }
            else {
                filledGrid[gx][gy] = 1;
...
void Menu(int index) {
    ...
    // Free triangle points if allocated
    if (trianglePoints != NULL) {
        free(trianglePoints);
        trianglePoints = NULL;
    }
...
int main(int argc, char **argv) {
    ...
    glutCreateWindow("113598017 - LAB9");
    glutCreateMenu(Menu);
